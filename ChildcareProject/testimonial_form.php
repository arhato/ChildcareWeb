...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($service) || empty($name) || empty($rdate)) {
    ...
    if (empty($err)) {
        if (!$row['count'] > 0) {
            $query = "INSERT INTO `user`(`testinomial`) 
            VALUES('$testinomial')";
            ...
            if ($result) {
                ...
                $success = "Review cannot be Added.";
            }
        }
    }
    mysqli_close($conn);
...
<body>
    ...
    <div class="heading" style="background:url(images/about-img.jpg) no-repeat">
        <h1>Share Us Your Experience With Us</h1>
    </div>
    ...
    <section class="booking">
        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST" class="book-form">
            ...
            <div class="flex">
                <div class="inputBox">
                    <span>Service:</span>
                    <select id="selector" name="service" class="selector" value="<?php echo isset($_POST['service']) ? htmlspecialchars($_POST['service']) : ''; ?>">
                ...
                <div class="inputBox">
                    <span>Date:</span>
                    <input type="date" name="rdate" value="<?php echo isset($_POST['rdate']) ? htmlspecialchars($_POST['rdate']) : ''; ?>">
                </div>
                <div class="inputBox">
                </div>
                <div class="inputBox">
                    <span>Review:</span>
                    <input type="text" name="review" placeholder="Maximum 512 Characters" style="height: 300px;">
                </div>
